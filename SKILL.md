...
6. Bad/missing response → `disputeDraw` on-chain and STOP (max loss = one draw). Good → `affirmDraw` on-chain (that IS the close, and it builds the seller's reputation). No refunds — reputation is your protection.
